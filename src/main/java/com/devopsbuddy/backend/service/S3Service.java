...
@Service /*no need to add @Transactional for there's no interaction with the database*/
public class S3Service {
	
	/** The application logger */
    private static final Logger LOG = LoggerFactory.getLogger(S3Service.class);
    /*used to name the file being stored locally, the file to be uploaded to Amazon s3*/
    ...
    private String tempImageStore;/*declared in ApplicationConfig */
    ...
    public String storeProfileImage(MultipartFile uploadedFile, String username) throws /*name from Amazon s3*/
    		...
    		try {
	    		if(uploadedFile != null && !uploadedFile.isEmpty()) {
	    			byte[] bytes = uploadedFile.getBytes(); /*creates an array of bytes from the multipart file*/
	    			...
	    			if (!tmpImageStoredFolder.exists()) {
	    				...
	    				tmpImageStoredFolder.mkdirs(); /*create folder if it doesn't exist*/
	    			}
	    			...
	    			/*full path to the file that contains the image*/
    		...
    		if (null == rootBucketUrl) {
    			...
    			try {
    				...
    				LOG.error("A client exception occurred while trying to store the profile" +
    						" image {} on S3. The profile image won't be stored", resource, ace);
